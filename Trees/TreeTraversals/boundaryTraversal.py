...
class Solution:
    ...
    def boundary_traversal(
        self, root
    ):  # TC O(H) for left boundary, O(N) for leaf nodes, O(H) for right boundary total O(N)
        if not root:
            return []
        ...
        left = self.left_boundary(
            root.left
        )  # left boundary(excluding leaf nodes and root)
        leaf = self.leaf_nodes(root)  # leaf nodes
        right = self.right_boundary(
            root.right
        )  # right boundary(excluding leaf nodes and root)
